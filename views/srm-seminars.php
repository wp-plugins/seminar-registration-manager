<div class="srm-seminars">
    <div class="srm-header">
        <h1>Manage <?php echo ucwords(strtolower(SRM_DEFAULT_NAME)); ?>s</h1>
        <?php echo SrmCommon::render_filter(true, true); ?>
        <div class="srm-back-link"><a href="<?php bloginfo('url');?>/wp-admin/admin.php?page=srm-seminars-add-edit">Create New <?php echo ucwords(strtolower(SRM_DEFAULT_NAME)); ?></a></div>
    </div>
    <?php
        if ($seminars_count==0):
    ?>
            <div class="error">There are no <?php echo strtolower(SRM_DEFAULT_NAME);?>s for the given view. Perhaps changing the filter options above will yield more results.</div>
    <?php     
        else:
    ?>
            <table class="srm-grid">
                <tr>
                    <th><?php echo ucwords(strtolower(SRM_DEFAULT_NAME)); ?></th>
                    <th>Date</th>
                    <th>Location</th>
                    <th>Attendees</th>
                    <th>Actions</th>
